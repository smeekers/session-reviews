...
- [ ] There should be no seeded session with the "processing status"
- [ ] I noticed "ready" sessions have a date? What are we using for date? It should be start time not creation time. Its fine if ready sessions are using a different value for date
- [ ] Banners are not dismissable if they have an action
- [ ] I want better mocked ai responses. More suggestions per session and more variety of summaries and suggestions (bigger pool)
